...
모든 {{site.data.keyword.Bluemix_notm}}의 데디케이티드 배치에는 추가 비용 없이 VPN, 사설 VLAN(Virtual Local Area Network), 방화벽, 기존 온프레미스 데이터베이스 및 앱을 활용하는 능력, 연중무휴 온사이트 보안, 데디케이티드 하드웨어, 표준 지원 등 여러 혜택과 기능이 포함됩니다.
...
고객 담당자는 IBM 전문가와 공동 작업을 통해 사용자가 항상 필요한 지원을 받을 수 있도록 보장합니다. 프리미엄 지원 계층으로 업그레이드하여 사용자 계정의 전담 기술 영업 담당자(TAM)와 함께 작업할 수 있습니다. 여러 지원 계층에 대한 자세한 정보는 [지원 유형](/docs/get-support/getstarttssup.html#typesofsupport)을 참조하십시오. TAM은 다음 유형의 태스크를 완료합니다.
